...
export const TabsList = ({ className, ...props }: TabsListProps) => (
  <TabsPrimitive.List
    className={cn('scrollbar-hide flex overflow-scroll border-b focus:outline-none', className)}
    {...props}
  />
)
...
export const TabsTrigger = ({ className, ...props }: TabsTriggerProps) => (
  <TabsPrimitive.Trigger
    className={cn(
      'data-[state=active]:border-primary data-[state=active]:text-primary flex h-12 flex-1 shrink-0 items-center justify-center border-b-2 border-transparent bg-white px-5 leading-none select-none focus:outline-none focus-visible:outline-none data-[state=active]:font-semibold',
      className,
    )}
    {...props}
  />
