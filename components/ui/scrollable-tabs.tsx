...
export const ScrollableTabsList = ({ className, ...props }: ScrollableTabsListProps) => (
  <TabsPrimitive.List
    className={cn(
      'scrollbar-hide relative flex overflow-x-scroll border-b focus:outline-none',
      className,
    )}
    {...props}
  />
...
export const ScrollableTabsTrigger = ({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className="data-[state=active]:border-primary data-[state=active]:text-primary h-10 shrink-0 border-b-2 border-transparent bg-white px-5 leading-none select-none focus:outline-none focus-visible:outline-none data-[state=active]:font-semibold"
      value={value}
      {...props}
    />
  )
