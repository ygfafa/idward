...
export const RadioGroupButtonItem = ({ block, className, ...props }: RadioGroupButtonItemProps) => (
  <RadioGroupPrimitive.Item
    className={cn(
      'text-muted-foreground data-[state=checked]:border-primary data-[state=checked]:bg-primary flex h-9 items-center justify-center rounded-[16rem] border px-4 data-[state=checked]:text-white',
      block && 'w-full',
      className,
    )}
    {...props}
...
export const RadioGroupItem = ({ className, ...props }: RadioGroupItemProps) => (
  <div className="flex items-center gap-2">
    <RadioGroupPrimitive.Item
      id={props.value}
      className={cn(
        'data-[state=checked]:border-primary data-[state=checked]:bg-primary h-5 w-5 cursor-default rounded-full border bg-white outline-none',
        className,
      )}
      {...props}
    >
    ...
    <label className="leading-none select-none" htmlFor={props.value}>
      {props.children}
    </label>
  </div>
)
